    ...
    if st.button("この内容でリマインダーを作成", type="primary", use_container_width=True):
        with st.spinner("🧠 AIが画像を解析し、カレンダー登録内容を作成しています..."):
            try:
                ...
                # AIへの指示プロンプト
                prompt = """
               あなたは、非常に優秀で観察眼の鋭い秘書です。
                このLINEのトーク画面の画像を分析し、Googleカレンダーに登録するためのリマインダー情報を作成してください。

                # 最重要ルール: 会話の役割分担
                - **左側の吹き出し**: これは「相手」からのメッセージです。
                - **右側の吹き出し**: これは「あなた（ユーザー自身）」からのメッセージです。
                - **メッセージの最後**: 最後のメッセージの内容は、「相手」からのメッセージです。
                **このルールを元に、誰が誰にお願いしているのか、どちらが返信すべき立場なのかを正確に判断してください。**

                # 指示
                1.  **相手の名前 (name)**: トーク画面の最上部に表示されている「相手の名前」を抽出してください。
                2.  **返信すべき内容の要約 (task)**: 「あなた（ユーザー自身）」が、次に取るべきアクションや返信すべき内容を簡潔にまとめてください。特に、相手からの質問や依頼（「～お願いします」「～どうですか？」など）に注目してください。
                3.  **カレンダーのタイトル (title)**: 「【LINE返信】〇〇さんへ」のように、上記で抽出した相手の名前を使ってタイトルを作成してください。
                4.  **カレンダーの詳細 (description)**: 上記で要約した返信すべき内容を、詳細説明として記述してください。

                # 出力形式 (JSON以外の説明は絶対に含めないこと)
                {
                  "name": "ここに抽出した相手の名前",
                  "task": "ここに返信すべき内容の要約",
                  "title": "ここに生成したカレンダーのタイトル",
                  "description": "ここに生成したカレンダーの詳細説明"
                }
                """
